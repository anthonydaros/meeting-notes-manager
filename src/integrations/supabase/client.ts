...
supabase.auth.getSession().then(({ data, error }) => {
  if (error) {
    ...
    if (data.session) {
      ...
      if (typeof data.session.expires_at === 'string') {
        // Convert string timestamp to number
        expiresAtTimestamp = new Date(data.session.expires_at).getTime();
      } else {
        // Already a number
        expiresAtTimestamp = data.session.expires_at || new Date().getTime();
      }
      
      const now = new Date().getTime();
      const timeLeft = expiresAtTimestamp - now;
      ...
      if (timeLeft < 60 * 60 * 1000) {
        ...
        supabase.auth.refreshSession().then(({ data: { session } }) => {
          if (session) {
            ...
            const newExpiresAtTimestamp = newExpiresAt.getTime();
            session.expires_at = newExpiresAtTimestamp;
